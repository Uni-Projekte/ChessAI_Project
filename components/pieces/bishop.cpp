...
void bishop::possibleMoves(MOVE_ARRAY &moves, BOARD &attackedFieldsWhite, BOARD &attackedFieldsBlack, BOARD allPieces, BOARD currentColor, uint8_t x, uint8_t y)
{
    // diagonal north east
    uint8_t newX = x + 1;
    uint8_t newY = y + 1;
    while (newX < 8 && newY < 8 && !(currentColor & SINGLE_BIT_BOARD(newX, newY)))
    {
        ...
        {
            ADD_MOVE(moves, attackedFieldsWhite, attackedFieldsBlack, x, y, newX, newY, CAPTURE);
            break;
        } else {
            ADD_MOVE(moves, attackedFieldsWhite, attackedFieldsBlack, x,y,newX,newY,0);
        }
        newX++;
        newY++;
    }
